...
const subtrair = (n1:number, n2:number)=> n1-n2 // se nao tiver chaves nao se pode colocar retorno 
...
function args(...args:number[]):number[] // ex rest so é possivel passar parametros eantres do spread depois da erro
{
    return args;
}
...
function tuplaParans1(a:number,b:string,c:boolean):void 
{
    console.log(`1) ${a},${b},${c}`)
}
...
function tuplaParam3(...params: [number,string,boolean]) {
...
const dizerOla =(nome:string)=> console.log(`Ola, ${!nome? 'Pessoa':nome}`);
dizerOla("Anna");
